...
public static class UdpProxy
{
	...
	public static bool StartProxy(string dstHost, int dstPort)
	{
		lock (_stateLock)
		{
			...
			try
			{
				...
				_localListener.Bind(new IPEndPoint(IPAddress.Loopback, 55555));
				_isRunning = true;
	...
	private static async Task WorkerLoop(IPAddress dstIp, int dstPort, CancellationToken token)
	{
		...
		try
		{
			while (!token.IsCancellationRequested && _localListener != null && _isRunning)
			{
				...
				try
				{
					socketReceiveFromResult = await _localListener.ReceiveFromAsync(buffer, SocketFlags.None, remoteEndPoint, token);
				}
				...
				if (socketReceiveFromResult.ReceivedBytes <= 0)
				{
					continue;
				}
				if (socketReceiveFromResult.ReceivedBytes >= 9 && buffer[0] == 78 && buffer[1] == 69 && buffer[2] == 80 && buffer[3] == 95 && buffer[4] == 78 && buffer[5] == 73 && buffer[6] == 67 && buffer[7] == 75 && buffer[8] == 58)
				{
					RbxmBridgeServer.RegisterClientNickname(Encoding.UTF8.GetString(buffer, 9, socketReceiveFromResult.ReceivedBytes - 9));
				}
				else
				{
					if (!(socketReceiveFromResult.RemoteEndPoint is IPEndPoint key))
					{
						continue;
					}
					ClientSession orAdd = _sessions.GetOrAdd(key, delegate(IPEndPoint ep)
					{
					...
					orAdd.LastActivity = DateTime.UtcNow;
					try
					{
						await orAdd.RemoteSocket.SendToAsync(new ArraySegment<byte>(buffer, 0, socketReceiveFromResult.ReceivedBytes), SocketFlags.None, dstEndPoint, token);
					}
					catch
					{
					}
	...
	private static async Task RelayRemoteToLocal(ClientSession session, CancellationToken token)
	{
		...
		try
		{
			while (!token.IsCancellationRequested && _isRunning && _localListener != null)
			{
				...
				try
				{
					socketReceiveFromResult = await session.RemoteSocket.ReceiveFromAsync(buffer, SocketFlags.None, remoteEndPoint, token);
				}
				catch (OperationCanceledException)
				{
					break;
				...
				if (socketReceiveFromResult.ReceivedBytes > 0)
				{
					...
					try
					{
						await _localListener.SendToAsync(new ArraySegment<byte>(buffer, 0, socketReceiveFromResult.ReceivedBytes), SocketFlags.None, session.ClientEndPoint, token);
					}
					catch
					{
					}
		...
		try
		{
			Task.Run(delegate
			{
				try
				{
					...
					IPAddress? obj2 = hostAddresses.FirstOrDefault((IPAddress a) => a.AddressFamily == AddressFamily.InterNetwork) ?? hostAddresses[0];
					IPEndPoint remoteEP = new IPEndPoint(obj2, dstPort);
					using Socket socket = new Socket(obj2.AddressFamily, SocketType.Dgram, ProtocolType.Udp);
					DisableConnReset(socket);
					byte[] bytes = Encoding.UTF8.GetBytes("NEP_NICK:" + nickname.Trim());
	...
	public static int WarmTunnel(string? dstHost = null, int dstPort = 0, int proxyPort = 55555, int packets = 5)
	{
		int num = 0;
		if (!string.IsNullOrEmpty(dstHost) && dstPort > 0)
		{
			try
			{
				...
				if (hostAddressesAsync.Wait(1000) && hostAddressesAsync.Result.Length != 0)
				{
					...
					DisableConnReset(socket);
					byte[] bytes = Encoding.UTF8.GetBytes("NEP_TUNNEL_WARMUP_V1");
					for (int i = 0; i < 3; i++)
					{
						try
						{
		...
		try
		{
			...
			byte[] bytes2 = Encoding.UTF8.GetBytes("NEP_PROXY_WARMUP_V1");
