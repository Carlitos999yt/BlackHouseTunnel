...
public static class LocalizationService
{
	...
	public static string DetectDefaultSystemLanguage()
	{
		try
		{
			string text = CultureInfo.CurrentUICulture.TwoLetterISOLanguageName.ToLower();
			return (text == "es") ? "es" : ((!(text == "pt")) ? "en" : "pt");
		}
		catch
		{
			return "en";
	...
	{
		string key2 = CurrentLanguage;
		if (!Translations.ContainsKey(key2))
		{
			key2 = "en";
		}
		if (Translations[key2].TryGetValue(key, out string value))
		{
			return value;
		}
		if (Translations["en"].TryGetValue(key, out string value2))
		{
			return value2;
		}
		return key;
