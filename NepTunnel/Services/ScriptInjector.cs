...
public static class ScriptInjector
{
	...
	public static (bool success, string message) InjectScriptIntoMap(string targetMapPath, string luauSource)
	{
		try
		{
			string contents = GenerateRbxmxScript("NepNameSyncScript", luauSource);
			string text = Path.Combine(ConfigManager.AppDataDir, "NepNameSyncScript.rbxmx");
			...
			if (!string.IsNullOrWhiteSpace(targetMapPath) && File.Exists(targetMapPath) && Path.GetExtension(targetMapPath).ToLowerInvariant() == ".rbxlx")
			{
				...
				if (text2.Contains("NepNameSyncScript"))
				{
					return (success: true, message: "✓ El script 'NepNameSyncScript' ya está inyectado en este mapa.");
				}
