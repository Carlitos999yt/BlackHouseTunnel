...
namespace BlackHouseTunnel.Services
{
    public static class RsmInstallerService
    {
        ...
        {
            string path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Roblox Studio");
        ...
        {
            string path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Roblox Studio Mod Manager");
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
            }
        ...
        public static async Task<bool> LaunchOfficialRsmBootstrapperAsync(Action<string, string> log)
        {
            ...
            if (!File.Exists(text))
            {
                log("Notice: No se encontró bundled_assets/RobloxStudioModManager.exe local.", "warn");
            }
            log("Abriendo ventana de configuración del Roblox Studio Mod Manager…", "ok");
            try
            {
                if (File.Exists(text))
                {
        ...
        public static async Task<bool> RepairFromGitHubRepoAsync(Action<string, string> log, Action<double> progress)
        {
            ...
            httpClient.DefaultRequestHeaders.Add("User-Agent", "BlackHouseTunnel/RepairEngine");
            ...
            int count = 0;
            int repaired = 0;
            foreach (string relPath in array)
            {
                count++;
