...
namespace BlackHouseTunnel.Services
{
    public static class RobloxStudioService
    {
        ...
        public static void LaunchServer(string studio, string port, string uid, string pg, string tg, string username = "Player")
        {
                ...
                if (proc != null)
                {
                    ...
                    proc.Exited += (s, e) =>
                    {
                        try
                        {
                            ...
                            if (exitCode != 0)
                            {
                                OnStudioError?.Invoke($"⚠ Roblox Studio Server finalizó con código 0x{exitCode:X8}", "err");
                            }
                        }
                        catch { }
                    };
            ...
            {
                OnStudioError?.Invoke($"✗ Error al lanzar Studio Server: {ex.Message}", "err");
            }
        }
        ...
        public static void LaunchClient(string studio, string server, string port, string pg, string tg, string inst = "StudioPlayer_Proxy", string username = "Player")
        {
                ...
                if (proc != null)
                {
                    ...
                    proc.Exited += (s, e) =>
                    {
                        try
                        {
                            ...
                            if (exitCode != 0)
                            {
                                OnStudioError?.Invoke($"⚠ Roblox Studio Client finalizó con código 0x{exitCode:X8}", "err");
                            }
                        }
                        catch { }
                    };
            ...
            {
                OnStudioError?.Invoke($"✗ Error al lanzar Studio Client: {ex.Message}", "err");
            }
        }
