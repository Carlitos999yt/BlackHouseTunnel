            ...
            if (request.HttpMethod == "GET")
            {
                switch (text)
                {
                    ...
                    case "/poll":
                    {
                        ...
                        {
                            SendJson(response, 200, new { status = "idle" });
                        }
                        else
                        {
                            SendJson(response, 200, new { status = "ready", name = bridgePending2, staging_dir = StagingDir });
                        }
                        break;
                    }
                    case "/download":
                    {
                        ...
                        {
                            SendJson(response, 404, new { error = "no file pending" });
                            break;
                        }
                        ...
                        {
                            SendJson(response, 404, new { error = "staged file missing or invalid" });
                            break;
                        }
                        try
                        {
            ...
            if (request.HttpMethod == "POST")
            {
                try
                {
                    ...
                    if (text == "/queue")
                    {
                        ...
                        {
                            SendJson(response, 400, new { error = "invalid file type: only .rbxm and .rbxmx supported" });
                        }
                        else if (!File.Exists(text5))
                        {
                            SendJson(response, 400, new { error = "file not found: " + text5 });
                        }
                        else
                        {
                            ...
                            SendJson(response, 200, new { status = "queued", staged = text7 });
                        }
                    }
                    else if (text == "/clear")
                    {
                        ...
                        SendJson(response, 200, new { status = "cleared" });
                    }
                    else
                    {
                        SendJson(response, 404, new { error = "not found" });
                ...
                {
                    SendJson(response, 500, new { error = ex2.Message });
                    return;
                }
            }
            SendJson(response, 404, new { error = "not found" });
